...
Meteor.publish('MeasumentsforMap', function(bottomLeft, topRight) {
  if (!bottomLeft && !topRight) {
    return [];
  }
  measurements = Measurements.find({location: { $exists: true }, location: {$geoWithin: {$box: [bottomLeft, topRight]}}});
  return measurements;
});
